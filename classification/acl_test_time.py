...
def evaluate(description):
    ...
    valid_settings = ["reset_each_shift",           # reset the model state after the adaptation to a domain
                      # train on sequence of domain shifts without knowing when a shift occurs
                      "continual",
                      "gradual",                    # sequence of gradually increasing / decreasing domain shifts
                      # consecutive test samples are likely to originate from different domains
                      "mixed_domains",
    ...
    model = eval(f'{adaptation_method_lookup(cfg.MODEL.ADAPTATION)}')(
        cfg=cfg, model=base_model, num_classes=num_classes)
    logger.info(
        f"Successfully prepared test-time adaptation method: {cfg.MODEL.ADAPTATION.upper()}")
    ...
    dom_names_loop = [
        "mixed"] if "mixed_domains" in cfg.SETTING else dom_names_all
    ...
    # start evaluation
    for i_dom, domain_name in enumerate(dom_names_loop[:1]):
        ...
        for severity in severities:
            ...
            logger.info(
                f"{cfg.CORRUPTION.DATASET} accuracy % [{domain_name}{severity}][#samples={len(test_data_loader.dataset)}]: {acc:.2%}")

    if len(errs_5) > 0:
        logger.info(
            f"mean error: {np.mean(errs):.2%}, mean error at 5: {np.mean(errs_5):.2%}")
    else:
